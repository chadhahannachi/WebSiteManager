...
//         await axios.patch(`http://localhost:5000/contenus/Solution/${currentSolution._id}`, currentSolution);
//         setSnackbar({
//           open: true,
...
//       setSnackbar({
//         open: true,
//         message: "Erreur lors de la sauvegarde de la solution.",
//         severity: "error",
//       });
//     }
//   };
...
//     try {
//       await axios.delete(`http://localhost:5000/contenus/Solution/${solutionToDelete}`);
//       setSnackbar({
//         open: true,
...
//         <img
//           src={params.value}
//           alt={params.row.titre || "Image de la solution"}
...
//   return (
//     <Box m="20px">
//       <Header title="SOLUTIONS" subtitle="Gestion des Solutions" />
...
//               {currentSolution._id ? "Modifier" : "Créer"}
//             </Button>
//           </Box>
//         </Box>
//       </Modal>
...
const SolutionsManagement = () => {
  ...
  const fetchSolutions = async () => {
    if (!token || !userId || !userEntreprise) {
      ...
      setSnackbar({
        open: true,
        message: "Données manquantes pour récupérer les solutions.",
        severity: "error",
      });
      setLoading(false);
      return;
  ...
  // Appeler fetchSolutions une fois que userEntreprise est défini
  useEffect(() => {
    if (userEntreprise) {
      fetchSolutions();
    }
  ...
  const handleSave = async () => {
    try {
      ...
      if (currentSolution._id) {
        await axios.patch(
          `http://localhost:5000/contenus/Solution/${currentSolution._id}`,
          currentSolution,
          config
        );
        setSnackbar({
        ...
        if (!currentSolution.entreprise) {
          throw new Error("L'entreprise de la solution n'est pas définie.");
        }
        await axios.post(
          "http://localhost:5000/contenus/Solution",
          currentSolution,
          config
        );
        setSnackbar({
      ...
      setSnackbar({
        open: true,
        message: error.message || "Erreur lors de la sauvegarde de la solution.",
        severity: "error",
      });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:5000/contenus/Solution/${solutionToDelete}`,
        config
      );
      setSnackbar({
        open: true,
  ...
  return (
    <Box m="20px">
      <Header title="SOLUTIONS" subtitle="Gérer les Solutions" />
