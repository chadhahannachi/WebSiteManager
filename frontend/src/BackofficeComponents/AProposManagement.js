...
const AProposManagement = () => {
  ...
  const [aproposToDelete, setAproposToDelete] = useState(null);
  ...
  const fetchAPropos = async () => {
    if (!token || !userId || !userEntreprise) {
      ...
      setSnackbar({
        open: true,
        message: "Données manquantes pour récupérer les sections À Propos.",
        severity: "error",
      });
      setLoading(false);
      return;
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: "Erreur lors de la récupération des sections À Propos.",
        severity: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleOpenDeleteDialog = (id) => {
    setAproposToDelete(id);
    setOpenDeleteDialog(true);
  };

  const handleCloseDeleteDialog = () => {
    setOpenDeleteDialog(false);
    setAproposToDelete(null);
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:5000/contenus/APropos/${aproposToDelete}`,
        config
      );
      setSnackbar({
        open: true,
  ...
  const columns = [
    ...
    { field: "description", headerName: "Description", flex: 3 },
    {
      ...
      renderCell: (params) => (
        <img
          src={params.value}
          alt={params.row.titre || "Image de la section À Propos"}
